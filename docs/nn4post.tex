...
As the figure \ref{figure: 1} hints, employing a large $N_c$ will
unnecessarily waste computational resource. So, instead we'd better try $N_c =
1, 2, 3, \ldots$ one by one, until increasing $N_c$ cannot reduce the loss
apparently. At this situation, e.g. $N_c = n$ for some $n$, it hints that the
posterior we are fitting has only $n$ apparent peaks. This reveals the
intrinsic nature of the posterior, and we shall stop increasing $N_c$ any
more, stop wasting the computational resource.\footnote{(A proposal:) Or
iteratively? That is, first training by $N_c = 1$; when loss becomes stable
after a period of training, add a new peak, so that $N_c = 1 \rightarrow 2$;
then, when loss becomes stable again after a new period of training, add a new
peak, so that $N_c = 2 \rightarrow 3$; repeating. Question: if so, then what
