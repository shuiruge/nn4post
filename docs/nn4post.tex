...
When enlarging the dataset, i.e. increasing $N_{D}$, the posterior $p ( \theta
;D )$, as the target function of variational inference, will be changed, since
we become more confident. Does it mean that we have to re-train all the
trained?

Indeed we have to keep training as appending these new data, but start at what
we have trained. Since we use gradient based optimizer, it may be beneficial
if tune $N_{D}$ progressively while adding new data progressively, making the
change smooth. The optimizer will not ``jump and get lost'' (like the case in
...
q ( \theta_{i} ; \lambda_{i} )$. This solves the provious problem. XXX
