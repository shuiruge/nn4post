...
\[ \langle f \rangle ( x ) \assign \mathbbm{E}_{\theta \sim p ( \theta |D )} [
   f ( x; \theta ) ] . \]
Since $q ( . )$ is an approximation to $p ( .|D )$, let $\langle f \rangle_{q}
( x ) \assign \mathbbm{E}_{\theta \sim q ( \theta )} [ f ( x; \theta ) ]$,
then the difference between them is $\delta \langle f \rangle ( x ) \assign
\langle f \rangle ( x ) - \langle f \rangle_{q} ( x )$.

\begin{theorem}
  We have the relation of order between the relative error of inference and
  loss
...
\begin{example}
  ...
  ``\tmcodeinline[shell]{/docs/curse\_of\_dimensionality.py}''.}
\end{example}
