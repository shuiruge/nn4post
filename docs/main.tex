...
\begin{theorem}
  ...
  \assign ( a, \mu , \zeta )$ for shot, and star notation for representing the
  trained) by Monte-Carlo integral
  \[ \langle g \rangle ( x ) = \frac{1}{C} \times \sum_{i=1}^{N_{s}} \left[
     \frac{\tilde{p} ( \theta_{i} |D )}{q ( \theta_{i} ;z_{\ast} )}  g ( x;
     \theta_{i} ) \right] , \]
  where $\{ \theta_{i} :i=1, \ldots ,N_{s} \}$ sampled from $q ( \theta
  ;z_{\ast} )$ and $C= \sum_{i=1}^{N_{s}} \tilde{p} ( \theta_{i} |D ) /q (
  \theta_{i} ;z_{\ast} )$.
\end{theorem}
