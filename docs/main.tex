...
\begin{theorem}
  Let $p ( \theta |D )$ the posterior, and $\tilde{p} ( \theta |D ) =C p (
  \theta |D )$ for arbitrary contant $C$, but unknown. For the arbitrary
  ``observable'' $y=g ( x; \theta )$, for $\forall x$, Bayesian inference
  gives prediction $\langle g \rangle ( x )$ defined as
  \[ \langle g \rangle ( x ) \assign \int \mathd \theta  p ( \theta |D )  g (
     x; \theta ) \]
  can be computed by the trained distribution $q ( \theta ;z_{\ast} )$ ($z
  \assign ( a, \mu , \zeta )$ for short, and star notation for representing
  the trained) by weighted Monte-Carlo integral
  \[ \langle g \rangle ( x ) \approx \sum_{s=1}^{N_{s}} [ w ( \theta_{s}
     ;z_{\ast} ,D ) \times g ( x; \theta_{s} ) ] , \]
  with
  \[ w ( \theta_{s} ;z_{\ast} ,D ) \assign \tmop{softmax}_{s} ( \eta ) \]
  where
  \[ \eta_{s} \assign \ln   \tilde{p} ( \theta_{s} |D ) - \ln  q ( \theta_{s}
     ;z_{\ast} ) , \]
  and $\{ \theta_{s} :s=1, \ldots ,N_{s} \}$ sampled from $q ( \theta
  ;z_{\ast} )$. The error of the approximation can be estimated as
  $\mathcal{O} \left( 1/ \sqrt{N_{s}} \right)$ and is independent of the
  dimension of the space of $\theta$.
\end{theorem}

\begin{proof}
  Since
  \[ \int \mathd \theta   \tilde{p} ( \theta |D ) =C \times \int \mathd \theta
     p ( \theta |D ) =C \times 1 \]
  and
  \[ \int \mathd \theta   \tilde{p} ( \theta |D ) = \int \mathd \theta  q (
     \theta ;z_{\ast} ) \frac{\tilde{p} ( \theta |D )}{q ( \theta ;z_{\ast} )}
     \approx \sum_{s=1}^{N_{s}} \frac{\tilde{p} ( \theta_{s} |D )}{q (
     \theta_{s} ;z_{\ast} )} \]
  where $\{ \theta_{s} :s=1, \ldots ,N_{s} \}$ sampled from $q ( \theta
  ;z_{\ast} )$, we get
  \[ C= \sum_{s=1}^{N_{s}} \frac{\tilde{p} ( \theta_{s} |D )}{q ( \theta_{s}
     ;z_{\ast} )} . \]
  Then,
  \begin{eqnarray*}
    \int \mathd \theta  p ( \theta |D )  g ( x; \theta ) & = & \frac{1}{C}  
    \int \mathd \theta   \tilde{p} ( \theta |D )  g ( x; \theta )\\
    & = & \frac{1}{C}   \int \mathd \theta  q ( \theta ;z_{\ast} )
    \frac{\tilde{p} ( \theta |D )}{q ( \theta ;z_{\ast} )}  g ( x; \theta )\\
    & \approx & \frac{1}{C} \times \sum_{i=1}^{N_{s}} \left[ \frac{\tilde{p}
    ( \theta_{s} |D )}{q ( \theta_{s} ;z_{\ast} )}  g ( x; \theta_{i} )
    \right] .
  \end{eqnarray*}
  Denoting
  \[ \eta_{s} \assign \ln   \frac{\tilde{p} ( \theta_{s} |D )}{q ( \theta_{s}
     ;z_{\ast} )} , \]
  and with some direct re-arrangements, we get
  \[ \int \mathd \theta  p ( \theta |D )  g ( x; \theta ) \approx
     \sum_{s=1}^{N_{s}} \left[ \frac{\exp ( \eta_{s} )}{\sum_{s' =1}^{N_{s}}
     \exp ( \eta_{s'} )} \times g ( x; \theta_{s} ) \right] \]
  which is what we want. In this proof, all the approximations come from
  Monte-Carlo integrals, whose error can thus be estimated as $\mathcal{O}
  \left( 1/ \sqrt{N_{s}} \right)$ which is independent of the dimension of the
  space of $\theta$.
\end{proof}

Notice that we did not compute the mean value $g ( x; \theta )$ directly, as
$\mathbbm{E}_{\theta \sim q ( \theta ;a_{\ast} , \mu_{\ast} , \zeta_{\ast} )}
[ g ( x; \theta ) ]$, which is not accurate enough since $q ( \theta ;z_{\ast}
)$ is just an approximation to $p ( \theta | D )$. Instead, our implementation
does help gain the accurate enough result of the Monte-Carlo integral, at the
same time avoids the non-convergence of MCMC by ``sampling by importance''.
...
Furthermore, MCMC is not greedy enough so that it converges quite slowly,
