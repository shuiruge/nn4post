...
\begin{theorem}
  ...
  the trained) by Monte-Carlo integral
  \[ \langle g \rangle ( x ) \approx \sum_{i=1}^{N_{s}} \left[ \frac{w (
     \theta_{i} ;z_{\ast} ,D )}{\sum_{j=1}^{N_{s}} w ( \theta_{j} ;z_{\ast} ,D
     )} \times g ( x; \theta_{i} ) \right] , \]
  with
  \[ w ( \theta_{i} ;z_{\ast} ,D ) \assign \frac{\tilde{p} ( \theta_{i} |D
     )}{q ( \theta_{i} ;z_{\ast} )} , \]
  where $\{ \theta_{i} :i=1, \ldots ,N_{s} \}$ sampled from $q ( \theta
...
\begin{proof}
  ...
  \[ \int \mathd \theta   \tilde{p} ( \theta |D ) = \int \mathd \theta  q (
     ...
     \approx \sum_{i=1}^{N_{s}} \frac{\tilde{p} ( \theta_{i} |D )}{q (
     \theta_{i} ;z_{\ast} )} \]
  where $\{ \theta_{i} :i=1, \ldots ,N_{s} \}$ sampled from $q ( \theta
  ;z_{\ast} )$, we get
  \[ C= \sum_{i=1}^{N_{s}} \frac{\tilde{p} ( \theta_{i} |D )}{q ( \theta_{i}
     ;z_{\ast} )} . \]
  Then,
  \begin{eqnarray*}
    ...
    ( \theta_{i} |D )}{q ( \theta_{i} ;z_{\ast} )}  g ( x; \theta_{i} )
    \right] .
  \end{eqnarray*}
  Denoting
  \[ w ( \theta_{i} ;z_{\ast} ,D ) \assign \frac{\tilde{p} ( \theta_{i} |D
  ...
  \[ \int \mathd \theta  p ( \theta |D )  g ( x; \theta ) \approx
     \sum_{i=1}^{N_{s}} \left[ \frac{w ( \theta_{i} ;z_{\ast} ,D
     )}{\sum_{j=1}^{N_{s}} w ( \theta_{j} ;z_{\ast} ,D )} \times g ( x;
     \theta_{i} ) \right] \]
