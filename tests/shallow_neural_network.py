...
with Timer():
    nn4post.fit(batch_generator=batch_generator,
            ...
            learning_rate=0.1,
            batch_ratio=1.0,
            logdir='../dat/graph/shallow_nn_{0}'.format(NUM_PEAKS),
            dir_to_ckpt='../dat/checkpoint/shallow_nn_{0}'.format(NUM_PEAKS),
            skip_steps=50,
    )
...
ax.set_title('Shallow Neural Network (hidden: 100)')
plt.show()
