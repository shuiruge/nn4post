...
..:code:
    ...
    def likelihood(params, y_obs):
        """:math:`\ln p(\themta \sim y_{obs})`"""
        y = model(inputs, params)
        Y = Normal(y, sigma)
        #return Y.log_prob(y_obs)
        return tf.reduce_sum(Y.log_prob(y_obs))
'''


def log_likelihood(params):
    """:math:`\ln p(\themta \sim y_{obs})`"""
    model_output = model(inputs=input_data, params=params)['y']
    Y = NormalWithSoftplusScale(model_output, y_err)
    return scale * tf.reduce_sum(Y.log_prob(y))
