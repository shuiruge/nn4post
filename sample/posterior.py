...
def get_param_shape(param_dict):
    ...
    Args:
        param_dict:
            `dict`, like `{'w': ..., 'b': ...}, with values being either
            instances of `tf.contrib.distributions.Distribution`, or any objects
            that have `shape` attribute (e.g. numpy array or TensorFlow tensor).
...
def get_parse_param(param_shape):
    ...
    Returns:
        ...
        Returns:
            `dict` with keys the same as `param_shape`, and values Tensors with
            shape the same as the values of `param_shape`.
    """
...
def get_log_posterior(model, input, observed, param_prior, scale=None):
    """
    Args:
        model:
            Callable, with
    ...
    log_likelihood = get_log_likelihood(
        model, input, observed, scale=scale)
...
if __name__ == '__main__':
    ...
    log_posterior = get_log_posterior(
        model, input, observed, param_prior, scale=scale)
